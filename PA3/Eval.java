...
public class Eval {
	...
	static double value_roll_hand(State s, int depth) {
		...
		//Looked up how to get stuff from enum
		//Microsoft gave me this: https://docs.microsoft.com/en-us/dotnet/api/system.enum.getvalues?view=net-5.0
	...
	static public double heuristic(State s) {
		...
		if(s.blasts_collected <= 2 && s.brains_collected > 0){
			if(s.comp_brains_eaten-s.user_brains_eaten > 1){
				value = s.win_payoff;
			}
			else value = -s.win_payoff;
		}
		else if(s.comp_brains_eaten >= s.user_brains_eaten){
			value = s.win_payoff;
		}
		else value = -s.win_payoff;
