...
// Used PA4hints.pdf given by David Noelle for literally the entire code
public class Layer {
	...
	public void computeOutputDelta() {
		// Page 4 in PA4hints.pdf
		// for outputs

		// Just moved down code to understand it
		// public double min; // minimum activation level
		// public double max; // maximum activation level
		// public Vector net; // net input levels of units
		// public Vector act; // activation levels of units
		...
		// First compute error between the target and the actual layers
		// Use difference() method to get target output - actual output
		// (Ti - ai)
		Vector TiMinusAi = targ.difference(act);
		// Get derivative of the net vector which has net input levels using min activation level and max activation level
		// g'(ini) | (ini) = (min, max)
		Vector gDerivative = net.derivative(min,max);

		int i = 0;
		for(i = 0; i < n; i++){
			// public Vector delta; // unit delta values
			// Delta gets value from multiplying errorDiff and netDerivative
			// delta = g'(ini)(Ti - ai)
			delta.set(i, TiMinusAi.get(i) * gDerivative.get(i));
		}
	...
	public void computeHiddenDelta() {
		// Page 4 to 11 in PA4hints.pdf
		// Otherwise
		// Pretty much the same as computeOutputDelta

		// Moved from above to understand
		// public int n; // number of units in the layer
		// public Vector net; // net input levels of units

		// Create a vector that allocates memory equal to the number of units n
		Vector errorDiff = new Vector(n, 0.0);
		// Taken from computeOutputDelta
		Vector gDerivative = net.derivative(min, max);

		// Vector multiplied by a matrix must be transposed to be a matrix multiplied by a vector
		for(Projection p: outputs){
			// public Matrix W; // the actual weight matrix
			// sum of matrix multiplications
			errorDiff = errorDiff.sum(p.W.transpose().product(p.output.delta));
		}

		// Taken from computeOutputDelta
		int i = 0;
		for(i = 0; i < n; i++){
			delta.set(i, errorDiff.get(i)*gDerivative.get(i));
		}
